...
const sortStarWarsCharacters = (starWarsArr) => starWarsArr.sort((value, index) => {
  return index.height - value.height;
});
...
const listFoods = (recipe) => {
  ...
  for ( let i=0; i < recipe.ingredients.length; i++){
    let newArr = recipe.ingredients[i].slice(recipe.ingredients[i].indexOf( ' ')+1);
    result.push(newArr.slice(newArr.indexOf(' ')+1));
  }
  return result;
};
